...
Кадры для `landing/index.html`. Снимать в светлой теме, ретина (двойной размер),
формат PNG. После того как файл лёг сюда, в `index.html` найти комментарий
«СКРИНШОТ N» и заменить блок `<div class="shot">…</div>` на строку из комментария.
