...
и складывает в словарь. По умолчанию работает только на сайтах из списка разрешённых
(там `reddit.com`), но список переворачивается в настройках: режим «везде, кроме»
включает расширение на всех сайтах, кроме перечисленных и заведомо чувствительных
(почта, банки, госуслуги, внутренние хосты).
...
npm version patch      # бамп версии + закрытие раздела «Не выпущено» в CHANGELOG
```
...
- UI — **только PrimeVue** + Tailwind 4, компоненты автоимпортируются (`PrimeVueResolver`),
  ...
  на страницы и оверлей, оба цвета — из иконки расширения), тёмная тема по классу
  `.dark`. Иконки — `lucide-vue-next`.
...
- **Никакого глобального CSS в документ страницы.** Оверлей рендерится в Shadow DOM, стили
  ...
  Единственное исключение — и только в Firefox-сборке — `margin-right` на `<html>`
  в `useOverlayDock`: док прижат к правому краю, и без отступа он закрывал бы текст.
  Ширина отступа и ширина дока приходят из одной константы (`DOCK_WIDTH`/`RAIL_WIDTH`),
  снимает отступ `releasePage()` при размонтировании оверлея.
- **Список слов в Chrome живёт в боковой панели браузера** (`chrome.sidePanel`,
  страница `src/ui/side-panel/`), у Firefox — док в странице; развилка — build-флаг
  `__HAS_SIDE_PANEL__` из `vite.chrome/firefox.config.ts`. Состояние разбора есть только
  у оверлея в content script: он публикует снимок и принимает команды через
  `utils/panelBus.ts` + `content-script/panelBridge.ts` (снимки слушает и background —
  бейдж на иконке). Словарь и настройки в сообщения не кладутся: они в `storage`
  и реактивны в обоих контекстах через `useBrowserStorage`. Всё, что трогает DOM
  страницы (подсветка, переход к слову, выбор области), выполняет оверлей по команде;
  открыть панель программно нельзя — только жестом пользователя: кнопка в попапе или
  плавающая кнопка на странице (жест доезжает до background сообщением `PANEL_OPEN`,
  `sidePanel.open` зовётся строго синхронно в обработчике). Пока панель открыта, она
  держит порт `PANEL_PORT` к вкладке — по нему оверлей прячет кнопку открытия.
...
- **Тема оверлея — класс `.dark` на `.nt-overlay` внутри shadow root**, не на `<html>` сайта:
  ...
  **`@theme inline`**: без `inline` `var(--p-*)` вычисляется на `:host` и вниз приходит светлым.
  Оттуда же дубль `--color-mark-new/saved`: в shadow root `assets/base.css` не приходит,
  и без него `bg-mark-*` не собирается вовсе.
- **Картинки — в `.svg`-файлах, а не в разметке.** Иллюстрации лежат в
  `assets/illustrations`, ожидания — в `assets/loaders`, вставляет их `InlineSvg`
  (единственный `v-html` в проекте, разметка своя, из сборки). Строкой (`?raw`),
  а не ссылкой: цвета внутри — токены темы и `currentColor`, у `<img>` они не
  вычисляются и картинка не перекрашивается под тёмную тему. Размер задаёт вызывающий,
  высоту svg берёт из `viewBox`. Классы и `@keyframes` внутри `.svg` идут с префиксом
  `nt-ld-`: `<style>` из файла попадает в документ целиком и действует на всё.
- **Ожидание рисует `AppLoader`, а не `LoaderCircle`.** Вариант выбирается по смыслу:
  `underline` — словарь ищет слово, `swap` — идёт перевод, `orbit` — всё остальное
  (модель, экспорт, разбор). Картинка декоративная: рядом всегда есть текст,
  озвучивается он.
- **Доступ к хостам выдаётся по одному, `<all_urls>` в манифесте нет** — иначе стор
  отправляет на ручную проверку, а установка пугает «доступом ко всем сайтам».
  Статически — только reddit, dictionaryapi.dev и MyMemory (он по умолчанию), остальное в
  `optional_host_permissions`. Content script в манифесте не объявлен:
  `background/siteScripts.ts` регистрирует его через `scripting.registerContentScripts`
  по выданным разрешениям (файл — импорт `?script`), пересобирает на `onInstalled`,
  `onStartup`, `permissions.onAdded/onRemoved` и по сообщению `SCRIPTS_SYNC`.
  Просит доступ `useHostAccess` — только из обработчика жеста и только на страницах
  расширения; новый сервис (переводчик, провайдер модели) обязан назвать свои хосты
  (`Translator.origins`, `baseUrl` модели), а его экран — показать `HostAccess`.
  Список сайтов синхронизируется через `storage.sync`, разрешения — нет.
  Dev-серверу поэтому нужен `server.cors: true`: код в dev грузится с localhost,
  а host permission на него больше нет.
...
- **Резервная копия снимает хранилища целиком, а не список полей.** `utils/backup.ts`
  кладёт в файл всё, что вернули `storage.sync.get()` и `storage.local.get()`, — новый
  ключ попадает в копию сам, перечислять его негде и незачем. Формат версионируется
  (`BACKUP_VERSION`), разбор строгий и бросает коды, а не текст (модуль тестируется
  в node). Экран — `options-page/pages/backup.vue`, он же предупреждает про ключи API
  в файле; после загрузки копии страницу перезагружает пользователь: `useBrowserStorage`
  мержит с дефолтами только при старте, а на лету через `onChanged` приходит голое
  значение из копии.
...
- **Словари — не модель.** Перевод одиночных слов и толкования идут во внешние словари
  (`src/utils/dictClient.ts`); перевод в карточке даёт источник из настроек — машинный
  переводчик (`lookupMachine`, источник `machine`) или Яндекс.Словарь по своему ключу,
  толкования всегда от dictionaryapi.dev; модель дороже и медленнее. Раскрытие карточки
  бесплатно, LLM-пояснение — только по кнопке.
- **Расширение работает сразу после установки, без ключей.** Кто ищет слова, решает
  ...
  и переводит выбранным переводчиком (по умолчанию MyMemory), `llm` зовёт модель. Развилка одна — `utils/analyze.ts`, её и
...
- **В прод-сборку не идёт ни один ключ.** Общий ключ Яндекс.Словаря убран: Яндекс
  закрыл выдачу новых ключей (формы редиректят на обратную связь), и раздавать свой
  на всех значило бы жить на одной квоте. Яндекс.Словарь остался в списке для тех,
  у кого ключ уже есть; условия сервиса требуют подписи с активной ссылкой везде,
  где показаны его данные, — `lookup.yandexAttribution`. Ключи модели — только в dev.
