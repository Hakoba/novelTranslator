...
- **Доступ к хостам выдаётся по одному, `<all_urls>` в манифесте нет** — иначе стор
  ...
  Статически — только reddit, словари и переводчик Edge (он по умолчанию), остальное в
...
- **Расширение работает сразу после установки, без ключей.** Кто ищет слова, решает
  ...
  и переводит выбранным переводчиком (по умолчанию Edge), `llm` зовёт модель. Развилка одна — `utils/analyze.ts`, её и
