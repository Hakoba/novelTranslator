...
- **Никакого глобального CSS в документ страницы.** Оверлей рендерится в Shadow DOM, стили
  ...
  Единственное исключение — `margin-right` на `<html>` в `useOverlayDock`: панель прижата
  к правому краю, и без отступа она закрывала бы текст вместо того, чтобы освободить место.
  Ширина отступа и ширина панели приходят из одной константы (`DOCK_WIDTH`/`RAIL_WIDTH`),
