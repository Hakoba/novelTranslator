...
- **Никаких строк интерфейса в коде.** Тексты — в `src/locales/{ru,en}.ts`, в компонентах
  `useI18n`, вне компонентов — `t` из `utils/i18n.ts`. Модули, которые тестируются в node
  (`anki.ts`), бросают код ошибки, а не текст: локали им недоступны. Русские формы
  числительных — свои `pluralRules` (у vue-i18n их две, нужно три).
- **Язык текста и язык интерфейса — разные настройки.** Пара «читаю на / перевод на»
  ...
  но список языков интерфейса свой и короче (`UI_LANGUAGES`).
